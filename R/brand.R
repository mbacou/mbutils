...
#'
#' Make this package compatible with `brand.yml` unified branding features. Branding
#' can be read from a configuration file provided at run-time, else it will use this
#' package built-in theme.
#'
...
#'
#' @param file path to `_brand.yml` configuration file, normally this file is
#' auto-detected in the working tree, but may be specified here to swap branding
#' dynamically.
#' @param font one of `_brand.yml` font families (currently only `base`, `monospace`,
#' or `headings`).
...
#'
#' This function is inspired by [thematic::thematic_on()]. It modifies base R
#' graphics, lattice, and ggplot colors and fonts to match a user-specified Bootstrap
#' theme (using `_brand.yml`) features. It also modifies the appearance of plot axes
#' and legends (see examples).
#'
#' Calling `brand_on()` has a few side effects in that it modifies global `par()`
#' parameters, ggplot theme and color palette, and it will silently mask generic
#' functions `graphics::plot`, `ggplot2::ggplot()` and `graphics::legend()'.
#'
#' Use in combination with [brand_off()] to restore the environment to its original
#' state.
#'
...
#'
#' Restore R session to its original state, discarding all color, font and plot
#' customizations.
#'
#' @seealso [brand_on()]
...
#' Bootstrap's **white**, **black**, **grid** and **gray** colors, which are
#' typically used for textual elements.
#'
...
#'
#' @export
brand.colors = function(x = NULL, omit = c("white", "black", "gray", "grid"), ...) {
